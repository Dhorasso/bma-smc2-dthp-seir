...
InitialState = [500000-1, 1, #####################################################################################
# Application of     BMA-SMC^2 for the Scenario B in the paper
# Note: All functions must be in the same folder.
#####################################################################################
...
theta_info_sir = {
    'gamma': {'prior': [0, 1, 0.16, 0.1, 'truncnorm', 'log']},  # Removal rate (Inverse of infectious period)
    'nu_beta': {'prior': [0.05, 0.15, 0.1, 0.05, 'truncnorm', 'log']},  # Beta variability
...
theta_info_dthp = {
    'omega_I': {'prior': [0, 1, 0.16, 0.1, 'truncnorm', 'log']},  # Decay parameter in the triggering kernel
    'nu_beta': {'prior': [0.05, 0.15, 0.1, 0.05, 'uniform', 'log']},  # Beta variability
...
# Improve overall appearance: grid, tight layout, etc.
fig.text(0.02, 0.97, 'B', fontsize=50, fontweight='bold', ha='center', va='center')
# Improve overall appearance: grid, tight layout, etc.
for ax in axes.flat:
    ax.grid(True, linestyle='--', alpha=0.9)  # Add grid with dashed lines
    ax.set_facecolor('whitesmoke')  # Add background color for each subplot
...
state_info_sir = {
    ...
    'R': {'prior': [0, 0, 0, 0, 'uniform']},  # Removed (Recovered or Deceased)
    'NI': {'prior': [0, 0, 0, 0, 'uniform']},  # Newly Infected
    'B': {'prior': [0, np.inf, 0.275, 0.01, 'normal']},  # Transmission rate (Beta)
}
...
state_info_dthp = {
    'NI': {'prior': [0, 3, 0, 0, 'uniform']},  # Newly Infected
    'C_I': {'prior': [0, 0, 0, 0, 'uniform']},  # Cumulative Infected
    'Rt': {'prior': [0, np.inf, 1.6, 0.05, 'normal']}  # Reproduction Number
}
...
ax.set_xlabel('Time (days)', fontsize=18, fontweight='bold', labelpad=10)
ax.set_ylabel('Model weights', fontsize=18, fontweight='bold', labelpad=10)
ax.set_title('Scenario B', fontsize=18, fontweight='bold', pad=15)
# Customize ticks
ax.tick_params(axis='both', which='major', labelsize=16)
ax.tick_params(axis='both', which='minor', labelsize=10)
ax.axvline(x=separation_point, color='black', linestyle='--', linewidth=2)
